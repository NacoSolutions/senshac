...
test("check-repo-cleanliness > fails if there is an empty directory", () => {
    ...
    const result = spawnSync("bun", ["run", "scripts/check-repo-cleanliness.ts"], { encoding: "utf8" });
    
    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("Found empty directories polluting the repository");
