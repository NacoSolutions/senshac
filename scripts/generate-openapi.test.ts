...
describe("generate-openapi", () => {
	test.skip("docs/openapi.yaml is in sync with src/server/handlers/index.ts", () => {
		const { content } = generate();
		const onDisk = readFileSync(resolve(REPO_ROOT, "docs/openapi.yaml"), "utf8");
		expect(onDisk).toBe(content);
	});

	test.skip("generated document parses as YAML and looks like OpenAPI 3.1", () => {
	...
	test.skip("convertPattern rewrites :param to {param} and emits a path parameter", () => {
	...
	test.skip("convertPattern dedupes repeated params and handles multi-param paths", () => {
	...
	test.skip("convertPattern leaves static paths untouched", () => {
		const { openapiPath, parameters } = convertPattern("/healthz");
		expect(openapiPath).toBe("/healthz");
		expect(parameters).toEqual([]);
	});

	test.skip("tagFor groups meta endpoints and uses first segment elsewhere", () => {
	...
	test.skip("buildDocument collapses methods on the same path into one path item", () => {
	...
	test.skip("buildDocument carries route comments into the operation description", () => {
		const doc = buildDocument(
			[
				{
					method: "GET",
	...
	test.skip("buildDocument emits both 200 and default responses for every operation", () => {
		const doc = buildDocument(
