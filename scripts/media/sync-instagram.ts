...
const RAW_BUCKET = process.env.R2_BUCKET || process.env.S3_BUCKET || "senshac-media-raw";
...
async function main() {
  ...
  if (!response.ok) {
    throw new Error(`Failed to fetch from Instagram API: ${await response.text()}`);
  }

  const data = await response.json();
  const posts = data.data || [];
  console.log(`Successfully fetched ${posts.length} total posts from Instagram account.`);
  ...
  try {
    const manifestRes = await fetch("https://media.senshac.com/instagram/posts.json");
  ...
  for (const post of posts) {
    ...
    if (tagLimits.size > 0 && !hasRoom) {
      console.log(`Skipping ${post.id} (reached CMS limit for all matching tags)`);
      continue;
    }
    ...
    if (existing) {
      const isOptimized = existing.media_url?.includes('.m3u8') || existing.media_url?.includes('.webp');
      if (isOptimized) {
        shouldSkip = true;
      }
    }
    ...
    try {
      ...
      const processAsset = async (source: string, isVideo: boolean, suffix = "") => {
        const ext = new URL(source).pathname.split('.').pop() || (isVideo ? 'mp4' : 'jpg');
        const id = `${post.id}${suffix}`;
        const r2Key = `instagram/${id}.${ext}`;
        
        const mediaResponse = await fetch(source);
        if (!mediaResponse.ok) throw new Error(`Failed to download media for ${post.id}`);
        ...
        await r2.send(
          new PutObjectCommand({
            ...
            ContentType: mediaResponse.headers.get("content-type") || (isVideo ? "video/mp4" : "image/jpeg"),
        ...
        if (process.env.GITHUB_TOKEN) {
          ...
          await fetch(`https://api.github.com/repos/NacoSolutions/senshac/dispatches`, {
             method: "POST",
             headers: {
           ...
           const fs = require('fs');
           if (fs.existsSync('.media-input')) fs.rmSync('.media-input', { recursive: true, force: true });
           if (fs.existsSync('media/processed')) fs.rmSync('media/processed', { recursive: true, force: true });
           fs.mkdirSync('.media-input');
           
           const sourcePath = `.media-input/source.${ext}`;
           
           console.log(`- Downloading...`);
           spawnSync("bun", ["scripts/media/download-r2.ts", RAW_BUCKET, r2Key, sourcePath], { stdio: "inherit" });
           
           console.log(`- Processing...`);
           spawnSync("bun", ["scripts/media/process-object.ts", sourcePath, "media/processed", r2Key], { stdio: "inherit" });
           
           console.log(`- Uploading...`);
           spawnSync("bun", ["scripts/media/upload-r2.ts", "media/processed", PROD_BUCKET], { stdio: "inherit" });
      ...
      if (post.media_type === "VIDEO") {
        if (post.media_url) publicUrl = await processAsset(post.media_url, true);
        if (post.thumbnail_url) thumbnailUrl = await processAsset(post.thumbnail_url, false, "_thumb");
      } else if (post.media_type === "CAROUSEL_ALBUM" && post.children?.data) {
        // Just use the first image as the main publicUrl if empty
        if (post.media_url) publicUrl = await processAsset(post.media_url, false, "_main");
        
        for (let i = 0; i < post.children.data.length; i++) {
          const child = post.children.data[i];
          if (child.media_url) {
             const childUrl = await processAsset(child.media_url, child.media_type === "VIDEO", `_${i}`);
             childrenUrls.push(childUrl);
          }
        }
        if (!publicUrl && childrenUrls.length > 0) publicUrl = childrenUrls[0];
      } else {
        if (post.media_url) publicUrl = await processAsset(post.media_url, false);
      }
  ...
  if (orphanedPosts.length > 0) {
    ...
    async function deletePrefix(bucket: string, prefix: string) {
      ...
      do {
        const listRes = await r2.send(new ListObjectsV2Command({ Bucket: bucket, Prefix: prefix, ContinuationToken: token }));
        if (listRes.Contents && listRes.Contents.length > 0) {
          await r2.send(new DeleteObjectsCommand({
            Bucket: bucket,
            Delete: { Objects: listRes.Contents.map((c: any) => ({ Key: c.Key })) }
          }));
          console.log(`Deleted ${listRes.Contents.length} objects for prefix ${prefix} in ${bucket}`);
        }
        token = listRes.NextContinuationToken;
      } while (token);
    }

    console.log(`Found ${orphanedPosts.length} orphaned posts to delete from R2...`);
    for (const orphaned of orphanedPosts) {
      console.log(`Pruning assets for post ${orphaned.id}...`);
      await deletePrefix(PROD_BUCKET, `videos/instagram/${orphaned.id}`);
      await deletePrefix(PROD_BUCKET, `images/instagram/${orphaned.id}`);
      await deletePrefix(RAW_BUCKET, `instagram/${orphaned.id}`);
    }
  }
