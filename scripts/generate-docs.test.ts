...
describe("generate-docs", () => {
	test.skip("docs/http-api.md is in sync with src/server/handlers/index.ts", () => {
		const { content } = generate();
		const onDisk = readFileSync(resolve(REPO_ROOT, "docs/http-api.md"), "utf8");
		expect(onDisk).toBe(content);
	});

	test.skip("extracts a healthy number of routes from the real handlers module", () => {
	...
	test.skip("preserves ordering caveats from leading // comments", () => {
		const { routes } = generate();
	...
	test.skip("extractRoutes handles single-line and multi-line entries", () => {
		const src = [
	...
	test.skip("extractRoutes attaches preceding // comments to the next entry", () => {
		const src = [
			"const ROUTE_TABLE: readonly RouteEntry[] = [",
	...
	test.skip("extractRoutes throws when ROUTE_TABLE is missing", () => {
		expect(() => extractRoutes("const SOMETHING_ELSE = [];")).toThrow(/ROUTE_TABLE/);
	});

	test.skip("groupRoutes groups by first path segment", () => {
	...
	test.skip("renderMarkdown produces an AUTO-GENERATED banner and route count", () => {
	...
	test.skip("renderMarkdown escapes pipe characters inside comments", () => {
		const md = renderMarkdown([
			{
				method: "GET",
				pattern: "/x",
